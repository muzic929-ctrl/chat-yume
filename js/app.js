...
const App = (() => {
  ...
  function initLockScreen() {
    ...
    function startDrag() { lockDragging = true; lockY = 0; ls.style.transition = 'none'; }
  ...
  function initWallpaper() {
    ...
    if (input) input.addEventListener('change', (e) => {
      ...
      reader.onload = (ev) => { applyWallpaper(ev.target.result); Storage.set('wallpaper', ev.target.result); UI.showToast('壁纸已更新'); };
  ...
  function openApp(app) {
    ...
    switch (app) {
      case 'chat':
        ...
        renderCharList(); break;
      case 'create':
        editingCharacterId = null; resetForm();
        document.getElementById('form-title').textContent = '角色卡';
        document.getElementById('btn-delete-char').style.display = 'none';
        document.getElementById('page-form').style.display = 'flex'; break;
      case 'settings':
        loadSettings();
        document.getElementById('page-settings').style.display = 'flex'; break;
    }
  }
  ...
  function updateStatusClock() {
    ...
    ['status-time'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = h + ':' + m; });
    ['widget-time'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = h + ':' + m; });
  ...
  function startParticles() {
    ...
    const particles = [];
    for (let i = 0; i < 40; i++) particles.push({ x: Math.random()*w, y: Math.random()*h, r: Math.random()*2.5+1, vx: (Math.random()-0.5)*0.3, vy: (Math.random()-0.5)*0.3-0.2, opacity: Math.random()*0.5+0.2 });
    function draw() {
      const bg = document.getElementById('wallpaper-bg');
          ...
          btn.addEventListener('click', async (e) => {
            ...
            if (char && AI.shouldStartConversation(char)) {
              ...
              if (settings.aiMode === 'real' && settings.apiKey) {
                try { opening = await AIAPI.sendMessage(settings, char, '（这是我们的第一次对话，请根据你的人设主动和我打招呼，说一句开场白）', []); }
                catch (e) { opening = AI.generateOpening(char); }
              } else { opening = AI.generateOpening(char); }
              Characters.addMessage(charId, { content: opening, type: 'received' });
              UI.showToast('已添加好友，TA 发来了消息');
            } else { UI.showToast('已添加好友'); }
            renderCharList();
          });
        });
      }
  ...
  function updateBadges(newFriendCount) {
    ...
    let unreadCount = 0;
    if (username) {
      Characters.getAll(username).filter(c => c.status === 'accepted').forEach(char => {
        unreadCount += Characters.getMessages(char.id).filter(m => m.type === 'received' && m.timestamp > (char.lastReadTime || '')).length;
      });
    }
    const total = unreadCount + newFriendCount;
    const appBadge = document.getElementById('app-badge');
    if (appBadge) { if (total > 0) { appBadge.style.display = 'flex'; appBadge.textContent = total > 99 ? '99+' : total; } else appBadge.style.display = 'none'; }
    const chatBadge = document.getElementById('chat-badge');
    if (chatBadge) { if (unreadCount > 0) { chatBadge.style.display = 'flex'; chatBadge.textContent = unreadCount > 99 ? '99+' : unreadCount; } else chatBadge.style.display = 'none'; }
    const dockBadge = document.getElementById('dock-badge');
    if (dockBadge) { if (newFriendCount > 0) { dockBadge.style.display = 'flex'; dockBadge.textContent = newFriendCount > 99 ? '99+' : newFriendCount; } else dockBadge.style.display = 'none'; }
  }
      ...
      input.onchange = (ev) => {
        ...
        reader.onload = (re) => {
          ...
          img.onload = () => {
            ...
            Characters.update(Auth.getCurrentUser(),Chat.getCurrentCharacterId(),{avatar:base64});
            Chat.setAvatar('chat-avatar',base64); UI.showToast('头像已更新');
  ...
  function saveSettings(){ Storage.set('settings_'+Auth.getCurrentUser(),getSettings()); }
  function loadSettings(){
    const s=Storage.get('settings_'+Auth.getCurrentUser(),{aiMode:'simulate'});
